...
{
	std::string msg;
	if (data.tour == 2 || data.tour == 5 || data.tour == 9)
	{
		print_title_joueur("MAINTENANCE TOUR " + std::to_string(data.tour), data);
		...
		for (std::vector<int>::iterator it = maintenance.begin(); it != maintenance.end(); it++)
		{
			...
			else if (data.tour == 5 && (*it == 4 || *it == 5 || *it == 3))
			{
				msg = "[Maintenance de l'ordinateur effectuée pour le tour " + std::to_string(data.tour) + ".]\n";
				printSlowly(msg, data);
				data.text_robot += msg;
			...
			else if (data.tour == 9 && (*it == 6 || *it == 7 || *it == 8 || *it == 9))
			{
				msg = "[Maintenance de l'ordinateur effectuée pour le tour " + std::to_string(data.tour) + ".]\n";
				printSlowly(msg, data);
				data.text_robot += msg;

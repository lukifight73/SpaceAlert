...
void infligeDegats(menace_externe *menace)
{
	...
	{
		menace->add_m_degats_str("[La " + menace->get_m_name() + " est déja morte.]\n");
		menace->add_m_degats_totaux_str(menace->get_m_degats_str());
	}
	// check imunity
	else if (menace->get_m_degats() > menace->get_m_etat_bouclier() && menace->get_m_degats() > 0)
	{
		...
		start_color(menace->get_m_zone(), *menace->get_m_zone()->getz_data());
		menace->add_m_degats_str("[La menace " + menace->get_m_name() + " absorbe " + std::to_string(menace->get_m_etat_bouclier()) + " dégats et reçois " + std::to_string(degatsInfliges) + " dégats.]\n");
		menace->add_m_degats_totaux_str(menace->get_m_degats_str());
		printSlowly(menace->get_m_degats_str(), *menace->get_m_zone()->getz_data());
		menace->get_m_zone()->getz_data()->text_robot += "[La menace " + menace->get_m_name() + " absorbe " + std::to_string(menace->get_m_etat_bouclier()) + " dégats et reçois " + std::to_string(degatsInfliges) + " dégats.]\n";
		menace->set_m_etat_bouclier(0); // Bouclier épuisé
		menace->recoitDegats(degatsInfliges); // Inflige les dégâts restants à la menace
		end_color(*menace->get_m_zone()->getz_data());
	}
	...
	{
		start_color(menace->get_m_zone(), *menace->get_m_zone()->getz_data());
		menace->add_m_degats_str("[La menace " + menace->get_m_name() + " absorbe " + std::to_string(menace->get_m_etat_bouclier()) + " dégats.]\n");
		menace->add_m_degats_totaux_str(menace->get_m_degats_str());
		printSlowly(menace->get_m_degats_str(),*menace->get_m_zone()->getz_data());
		end_color(*menace->get_m_zone()->getz_data());
	}
...
void	play_tour(t_data &data)
{
	...
	int num_joueur(1);
	print_tour("TOUR " + std::to_string(data.tour), data);
	apparitionMenaces(data);
	clear_actionUsage(data);
	actionMenaceDebutTour(data);
	print_title_joueur("ACTION DES JOUEURS", data);
	while (num_joueur <= data.nb_joueur)
	{
		start_color_joueur(data);
		std::string msg = "\n									---     " + data.joueurs[num_joueur]->getj_nom() + "     ---									\n";
		data.text_html += msg;
		end_color(data);
		start_color(data.zones[data.joueurs[num_joueur]->getj_zone()], data);
	...
	print_title("CALCUL DES DEGATS", data);
...
void	play_game(t_data &data)
{
	if (data.tour <= 12)
	{
		...
		clear_actionUsage(data);
		actionMenaceDebutTour(data);
		print_title("ACTION DES JOUEURS", data);
		while (num_joueur <= data.nb_joueur)
		{
			std::cout << "									------									\n";
			start_color(data.zones[data.joueurs[num_joueur]->getj_zone()], data);

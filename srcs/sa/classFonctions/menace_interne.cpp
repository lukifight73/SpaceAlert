...
{
    m_degats_str += "[" + joueur->getj_nom() + " attaque la menace interne " + m_name + " et lui inflige un dégat!]\n";
    m_degats++;
}
...
void menace_interne::getDamageHero(joueur *joueur)
{
    ...
    m_degats_str += "[ACTION HEROIQUE! " + joueur->getj_nom() + " attaque la menace interne " + m_name + " et lui inflige un dégat!]\n";
    m_degats++;
}

void menace_interne::getDamaged()
{
    start_color_interne(*m_zone->getz_data());
...
{
    std::string msg = "[La menace " + m_name + " inflige " +  std::to_string(degats) + " dégats a la " + m_zone->getz_nom_zone() + " .]\n";
    m_zone->getz_data()->text_robot += msg;
    printSlowly(msg, *m_zone->getz_data());
...
void menace_interne_i1_01::getDamage(joueur *joueur)
{
    ...
    if(m_vie <= m_degats)
    {
        std::string msg = "[La menace internce " + m_name + " riposte et désactive les robots de " + joueur->getj_nom() + ".]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
...
{
    joueur->setj_bots(joueur->getj_bots()); // pour eviter probleme de compilation
    m_degats_str += "[ACTION HEROIQUE! " + joueur->getj_nom() + " attaque la menace interne " + m_name + " et lui inflige deux dégat!]\n";
    m_degats++;
    m_degats++;
}
...
void menace_interne_i1_03::actionMenace(char input)
{
    ...
    else if (input == 'Y')
    {
        ...
        if(m_zone->getz_reacteur() > 0)
        {
            ...
            msg = "[La menace " + m_name + " consume 1 d'énergie au réacteur de la " + m_zone->getz_nom_zone() + ", il reste " + std::to_string(m_zone->getz_reacteur()) + " énergie au réacteur ..]\n";
            m_zone->getz_data()->text_robot += msg;
            printSlowly(msg, *m_zone->getz_data());
        }
        else
...
void menace_interne_i1_04::actionMenace(char input)
{
    ...
    else if (input == 'Y')
    {
        ...
        {
            m_zone->setz_reacteur(m_zone->getz_reacteur() -1);
            msg = "[La menace " + m_name + " consume 1 d'énergie au réacteur de la " + m_zone->getz_nom_zone() + ", il reste " + std::to_string(m_zone->getz_reacteur()) + " énergie(s) au réacteur ..]\n";
            m_zone->getz_data()->text_robot += msg;
            printSlowly(msg, *m_zone->getz_data());
        }
        else
...
void menace_interne_i1_05::actionMenace(char input)
{
    if (input == 'X')
    {
        ...
        std::string msg = "[La menace " + m_name + " draine toute l'énergie du bouclier de la " + m_zone->getz_nom_zone() + " .]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
    }
    else if (input == 'Y')
    {
        ...
        std::string msg = "[La menace " + m_name + " draine toute l'énergie du réacteur de la " + m_zone->getz_nom_zone() + " .]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
    }
    else if (input == 'Z') {
...
void menace_interne_i2_05::actionMenace(char input)
{
    if (input == 'X')
    {
        ...
        std::string msg = "[La menace " + m_name + " draine un d'énergie a chaque réacteur.]\n"; //TODO
        printSlowly(msg, *m_zone->getz_data());
        m_zone->getz_data()->text_robot += msg;
    }
    else if (input == 'Y')
    {
        ...
        std::string msg = "[La menace " + m_name + " inflige 1 dégats sur toutes les zones...]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
        m_zone->getdegatsIgnoreBouclier(1);
...
void menace_interne_si1_01::actionMenace(char input)
{
    ...
    else if (input == 'Y')
    {
        ...
        std::string msg = "[La menace " + m_name + " inflige 4 dégats a la " + m_zone->getz_nom_zone() + " et assome les joueurs qui y sont presents dans sa station.]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
        m_zone->getdegatsIgnoreBouclier(4);
...
void menace_interne_si1_03::actionMenace(char input)
{
    ...
    else if (input == 'Y')
    {
        ...
        std::string msg = "[La menace " + m_name + " monte en " + m_zone->getz_nom_zone() + " et inflige " + std::to_string(joueurCount) + " dégats,]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
        ...
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
        m_zone->getz_data()->vaisseau_en_vie = false;
    } else {
        std::cerr << "Action inconnue: " << input << std::endl;
    }
}
...
void menace_interne_si1_04::actionMenace(char input)
{
    if (input == 'X')
    {
        ...
        std::string msg = "[Tant que la menace " + m_name + " est présente, les dégats en zone rouge sont doublés!]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
    }
    else if (input == 'Y')
    {
        ...
        std::string msg = "[Tant que la menace " + m_name + " est présente, les dégats sur tout le vaisseau sont doublés!]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
    }
    else if (input == 'Z') {
        std::string msg = "[La menace " + m_name + " emporte et détruit le vaisseau en piece détachées...]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
        m_zone->getz_data()->vaisseau_en_vie = false;
    } else {
...
void menace_interne_si1_04::actionQuandDetruit()
{
    ...
    std::string msg = "[La menace " + m_name + " est éliminée! Les dégats recu par le vaisseau ne sont plus doublés!]\n";
    m_zone->getz_data()->text_robot += msg;
    printSlowly(msg, *m_zone->getz_data());
    menace::actionQuandDetruit();
}
...
void menace_interne_si1_05::actionMenace(char input)
{
    if (input == 'X')
    {
        ...
        if(EnergieBouclier > m_zone->getz_max_energie_bouclier())
        {
            ...
            std::string msg = "[La menace " + m_name + " transfére l'énergie du réacteur central au bouclier, le surplus se volatilise et cause "
            + std::to_string(EnergieBouclier - m_zone->getz_max_energie_bouclier()) + " dégats.]\n";
            m_zone->getz_data()->text_robot += msg;
            printSlowly(msg, *m_zone->getz_data());
        ...
        else
        {
            ...
            std::string msg = "[La menace " + m_name + " transfére l'énergie du réacteur central au bouclier sans causer de dégats.]\n ";
            m_zone->getz_data()->text_robot += msg;
            printSlowly(msg, *m_zone->getz_data());
        }
    ...
    {
        int EnergieReacteur = m_zone->getz_reacteur();
        std::string msg = "[La menace " + m_name + " fait se volatiliser l'énergie du réacteur, causant " + std::to_string(EnergieReacteur) + " dégats.]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
        m_zone->setz_reacteur(0);
        ...
        std::string msg = "[La menace " + m_name + " fait se volatiliser l'énergie des réacteurs, causant "
        + std::to_string(EnergieReacteur) + " dégats a la " + m_zone->getz_nom_zone() + ", " +
        std::to_string(EnergieReacteurR) + " dégats a la " + m_zone->getzone_right()->getz_nom_zone() + ", " +
        std::to_string(EnergieReacteurL) + " dégats a la " + m_zone->getzone_left()->getz_nom_zone() +
        ".]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
        m_zone->setz_reacteur(0);
...
void menace_interne_si1_06::getDamage(joueur *joueur)
{
    ...
    {
        m_degats_str += "[Vous avez infliges un dégat a la menace " + m_name + " dans chaque zone ou elle se trouve lors du même tour!.]\n";
        m_degats_str += "[--> +1 dégats!!!]\n";
        m_degats++;
    }
...
void menace_interne_si1_06::getDamageHero(joueur *joueur)
{
    ...
    if (m_degats_tour_zone != joueur->getj_zone() && m_degats_tour_zone != 0)
    {
        m_degats_str += "[Vous avez infligés un dégat a la menace " + m_name + " dans chaque zone ou elle se trouve lors du même tour!.]\n";
        m_degats_str += "[--> +1 dégats!!!]\n";
        m_degats++;
    }
    m_degats_tour_zone = joueur->getj_zone();
...
void menace_interne_si2_02::actionMenace(char input)
{
    ...
    else if (input == 'Y')
    {
        ...
        std::string msg = "[La menace " + m_name + " inflige 5 dégats a la " + m_zone->getz_nom_zone() + " et assome les joueurs de sa station .]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
...
void menace_interne_si2_03::actionMenace(char input)
{
    ...
    if (input == 'X')
    {
        ...
        std::string msg = "[La menace " + m_name + " consume 2 blocs d'énergie du réacteur central, il en reste " + std::to_string(m_zone->getzone_white()->getz_reacteur())
        + " .]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
    }
    else if (input == 'Y')
    {
        ...
        std::string msg = "[La menace " + m_name + " consume 1 blocs d'énergie de chaque réacteur, il en reste "
        ...
        std::string msg = "[La menace " + m_name + " inflige 3 dégats a chaque zones.]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
...
void menace_interne_si2_04::actionMenace(char input)
{
    ...
    {
        std::string msg = "[La menace " + m_name + " inflige 1 dégat dans chaque zone contaminee.]\n";
        m_zone->getz_data()->text_robot += msg;
        printSlowly(msg, *m_zone->getz_data());
        if (m_contamination_blue_haut)
        {
...
void menace_interne_si2_05::getDamage(joueur *joueur)
{
    ...
    {
        m_degats_str += "[Vous avez infliges 3 dégat a la menace " + m_name + " pendant le même tour!! Elle prends donc 1 dégats et meurt!]\n";
        m_degats++;
    }
    else
    {
        m_degats_str += "[" + joueur->getj_nom() + " attaque la menace interne " + m_name + "]\n" + "[" + std::to_string(3 - m_degats_par_tour) + " attaque(s) de plus lors de ce tour et la menace prendra un dégat.]\n";
    }
}

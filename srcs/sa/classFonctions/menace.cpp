...
{
    std::string msg = "[Attention, la menace " + m_name + " vient d'arriver et est présente en " + m_zone->getz_nom_zone() + "!]\n";
...
void menace::actionQuandDetruit() {
    ...
    m_zone->getz_data()->text_robot += "[La menace " + m_name + " en " + m_zone->getz_nom_zone() + " a été détruite.]\n";
    printSlowly(msg, *m_zone->getz_data());
...
void menace::recoitDegats(int input)
{
    ...
    {
        start_color(m_zone, *m_zone->getz_data());
        std::string msg = "[La menace " + m_name + " est déja detruite.]\n";
        printSlowly(msg, *m_zone->getz_data());
    ...
    m_zone->getz_data()->text_robot += "[La menace " + m_name + " a reçu " + std::to_string(input) + " points de dégâts. Vie restante : " + std::to_string(m_vie) + "]\n";
    if (m_vie == 0) {
        actionQuandDetruit();

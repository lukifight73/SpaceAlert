...
{
    std::string msg = "[" + j_nom + " est assomé jusqu'a la fin du voyage...]\n";
    j_data->text_robot += msg;
...
void joueur::attaqueIntercepteur(t_data &data)
{
    ...
    if (menace_fissure)
    {
        ...
        menace_fissure->add_m_degats_totaux_str(degats_str);
        j_data->text_robot += degats_str;
...
void joueur::setcarteTour(int tour, int action)
{
    cartes[tour].setc_action(action);
}

...
void mouvement_menaces(t_data &data)
{
    int j = 0;
    int i = 1;
    print_title_menaces("ACTIONS MENACES", data);
    while (i < 4)
    {
        std::vector<menace_externe*> tmp = data.zones[i]->getz_chemin_menace()->get_menacesExte();
        for (std::vector<menace_externe*>::iterator it = tmp.begin(); it != tmp.end(); ++it)
        {
            ...
            {
                //wait();
                start_color((*it)->get_m_zone(), data);
                ...
                msg = "[La menace " + (*it)->get_m_name() + " avance et se en position " + std::to_string(positionAfter) + ".]\n";
                data.text_robot += msg;
    ...
    if (!j)
    {
        std::string msg_mouvement_menace = "[Aucune menace n'est presente a proximité!]\n";
        printSlowly(msg_mouvement_menace, data);
        data.text_robot += msg_mouvement_menace;
    }
    std::cout << std::endl;
    end_color(data);
...
{
    print_title_menaces("APPARITION DES MENACES", data);
    int i = 1;
    while (i < 4)
    {
        std::vector<menace_externe*> tmp = data.zones[i]->getz_chemin_menace()->get_menacesExte();

...
/*   Updated: 2025/07/02 13:18:20 by lbenatar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void executeRequest(std::vector<t_serveur> &serversPort, t_client &client,
    ...
    //printHttpRequest(client.request);
	...
	else if (current_location.cgi_bool) //CGI
	{
        ...
        if (check_get_SA(client, dataStruct))
            return;
		handleGet(current_serveur, client, current_location);
	}
	else if (client.request.getMethod() == "POST") {
        if (check_post_SA(client, current_location, dataStruct))
            return;
		handlePost(current_serveur, client, current_location, dataStruct);
	}
	else
    {
